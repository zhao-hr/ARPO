...
def main(path, detail=False, std=False, start_step=240, end_step=250):
    ...
    for file in files:
        if file.endswith("_scores.npy"):
            ...
            start_step = start_step if end_step <= scores.shape[1] else scores.shape[1] - (end_step - start_step)
            end_step = end_step if end_step <= scores.shape[1] else scores.shape[1]
...
if __name__ == "__main__":
    ...
    parser.add_argument("-e", "--exp", default=["gradient_ascent", "coms", "iom", "romo", "gdr", "adr", "arpo"])
    parser.add_argument("-v", "--env", default="tfbind8")
