...
class AdaptiveRetrievalEnhancedMBO(nn.Module):
    ...
    def gradient_optimize(self, x, steps, pool):
        ...
        def gradient_step(xt):
            ...
            tmp_score, _ = self.forward_model(xt, tmp_retrieval_set)
            tmp_loss = self.entropy_coefficient * entropy + tmp_score
    ...
    def gradient_search_engine(self, x, pool, grad, batch_size=128):
        """
        x: [batch_size, x_dim(, x_cat)]
        pool_x: [pool_size, x_dim(, x_cat)]
        pool_y: [pool_size, y_dim]
        retrieval_set: [batch_size, size_retrieval_set, x_dim + y_dim(, x_cat)]
        """
        ...
        candidates_num = self.size_retrieval_set * 4
        indices = indices[:, :candidates_num]
        retrieval_x = pool_x[indices]
        retrieval_y = pool_y[indices]

        # 尽可能选择梯度方向的neighbors
        directions = (retrieval_x - x.unsqueeze(1).expand(-1, candidates_num, -1))
        ...
        retrieval_x = torch.gather(retrieval_x, 1, indices.unsqueeze(-1).expand(-1, -1, pool_x.shape[-1]))
        retrieval_y = torch.gather(retrieval_y, 1, indices.unsqueeze(-1).expand(-1, -1, pool_y.shape[-1]))
        if discrete_flag:
    ...
    def average_search_engine(self, x, pool, batch_size=128):
        """
        x: [batch_size, x_dim(, x_cat)]
        pool_x: [pool_size, x_dim(, x_cat)]
        pool_y: [pool_size, y_dim]
        ...
        # 选择尽可能不同的方向的neighbos
        directions = (retrieval_x[:,:,:x.shape[-1]] - x.unsqueeze(1).expand(-1, candidates_num, -1))
    ...
    def vanilla_search_engine(self, x, pool, batch_size=128):
        """
        x: [batch_size, x_dim(, x_cat)]
        pool_x: [pool_size, x_dim(, x_cat)]
        pool_y: [pool_size, y_dim]
