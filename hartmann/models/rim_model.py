...
class RetrievalInteractionMachineClassifier(nn.Module):
    ...
    def __init__(self,
        ...
        super(RetrievalInteractionMachineClassifier, self).__init__()
        self.forward_model = forward_model
        self.forward_model_optim = forward_model_optim(
            self.forward_model.parameters(),
            lr=forward_model_lr,
    ...
    def train_step(self, x, y, pool):
        ...
        # calculate the prediction error and accuracy of the model
        d = self.forward_model(x, retrieval_set)  # [batch_size, output_dim]
        ce = F.cross_entropy(d, y)
        statistics[f"train/loss_ce"] = ce.detach().unsqueeze(dim=0)
        ...
        # build the total loss and weight by the bootstrap
        total_loss = torch.mean(ce) + multiplier_loss
    ...
    def validate_step(self, x, y, pool):
        ...
        d = self.forward_model(x, retrieval_set)  # [batch_size, output_dim]
        ce = F.cross_entropy(d, y)
        statistics[f"validate/loss_ce"] = ce.detach().unsqueeze(dim=0)
