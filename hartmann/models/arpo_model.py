...
class AdaptiveRetrievalEnhancedMBO(nn.Module):
    ...
    def __init__(self,
        ...
        super(AdaptiveRetrievalEnhancedMBO, self).__init__()
    ...
    def vanilla_optimize(self, x, steps, pool):
        ...
        def gradient_step(xt):
            ...
            retrieval_set = self.vanilla_search_engine(xt, pool)
    ...
    def gradient_optimize(self, x, steps, pool):
        """Using gradient ascent find adversarial versions of x
        that maximize the conservatism of the model
        Args:
            x: torch.Tensor
        ...
        def gradient_step(xt):
            ...
            # the predicted scaled negative variance according to the forward model
            tmp_retrieval_set = self.vanilla_search_engine(xt, pool)
            tmp_score, _ = self.forward_model.inference(xt, tmp_retrieval_set, distil_grad=self.distil_grad, aggr_grad=self.aggr_grad)
            tmp_loss = self.entropy_coefficient * entropy + tmp_score # the conservatism of the current set of particles
            tmp_loss.backward(torch.ones_like(tmp_loss), retain_graph=True)
            tmp_grad = xt.grad

            xt.requires_grad_(False)

            retrieval_set = self.gradient_search_engine(xt, pool, tmp_grad)
            score, _ = self.forward_model.inference(xt, retrieval_set, distil_grad=self.distil_grad, aggr_grad=self.aggr_grad)
            loss = self.entropy_coefficient * entropy + score # the conservatism of the current set of particles
            loss.backward(torch.ones_like(loss), retain_graph=True)
            grad = xt.grad
    ...
    def average_optimize(self, x, steps, pool):
        """Using gradient ascent find adversarial versions of x
        that maximize the conservatism of the model
        Args:
            x: torch.Tensor
        ...
        def gradient_step(xt):
            ...
            retrieval_set = self.average_search_engine(xt, pool)
            score, _ = self.forward_model.inference(xt, retrieval_set, distil_grad=self.distil_grad, aggr_grad=self.aggr_grad)
            loss = self.entropy_coefficient * entropy + score # the conservatism of the current set of particles
            loss.backward(torch.ones_like(loss), retain_graph=True)
            grad = xt.grad
    ...
    def gradient_search_engine(self, x, pool, grad, batch_size=4096):
        """
        x: [batch_size, x_dim]
        pool[0]: [pool_size, x_dim]
        pool[1]: [pool_size, y_dim]
        retrieval_set: [batch_size, size_retrieval_set, x_dim + y_dim]
        """
        candidates_num = int(4 * self.size_retrieval_set)
        if self.retrieval_method == "distance":
            ...
            indices = indices[:, :candidates_num]
            ...
            indices = indices[:, :candidates_num]
            retrieval_set = torch.cat([pool_x[indices], pool_y[indices]], dim=-1)
        directions = (retrieval_set[:,:,:x.shape[-1]] - x.unsqueeze(1).expand(-1, candidates_num, -1))
        direction_sim = F.cosine_similarity(
            directions.reshape(x.shape[0] * candidates_num, -1),
            grad.unsqueeze(1).expand(-1, candidates_num, -1).reshape(x.shape[0] * candidates_num, -1),
            dim=-1).reshape(x.shape[0], candidates_num)
        _, indices = torch.sort(direction_sim, dim=-1, descending=True)
        indices = indices[:, :self.size_retrieval_set]
        retrieval_set = torch.gather(retrieval_set, 1, indices.unsqueeze(-1).expand(-1, -1, pool_x.shape[-1] + pool_y.shape[-1]))
        return retrieval_set
    
    def average_search_engine(self, x, pool, batch_size=4096):
        """
        x: [batch_size, x_dim]
        pool[0]: [pool_size, x_dim]
        pool[1]: [pool_size, y_dim]
        retrieval_set: [batch_size, size_retrieval_set, x_dim + y_dim]
        """
        candidates_num = int(3 * self.size_retrieval_set)
        if self.retrieval_method == "distance":
            pool_x, pool_y = pool[0], pool[1]
            num_batches = (pool_x.shape[0] // batch_size) + 1
            distance = []
            ...
            retrieval_set = torch.cat([pool_x[indices], pool_y[indices]], dim=-1)
        directions = (retrieval_set[:,:,:x.shape[-1]] - x.unsqueeze(1).expand(-1, candidates_num, -1))
        direction_sim = F.cosine_similarity(directions.unsqueeze(1), directions.unsqueeze(1), dim=3).reshape(x.shape[0], -1)
        _, indices = torch.sort(direction_sim, descending=False, dim=-1)
        rows_indices = indices // candidates_num
        cols_indices = indices % candidates_num
        indices = torch.stack([rows_indices, cols_indices], dim=2).reshape(x.shape[0], -1)
        unique = []
        for row in indices:
            unique_row = []
            seen = set()
    ...
    def vanilla_search_engine(self, x, pool, batch_size=4096):
        """
        x: [batch_size, x_dim]
        pool[0]: [pool_size, x_dim]
        pool[1]: [pool_size, y_dim]
    ...
    def train_step(self, x, y, pool):
        ...
        retrieval_set = self.vanilla_search_engine(x, pool)
