from models.forward_model import ROMO, DimensionROMO, RIM
...
def arpo(**config):
    ...
    # -----------------------------------------------------------
    logger.logger.info(f"========== Guide Forward Model ==========")
    # compute the normalized learning rate of the model
    guide_particle_lr = particle_lr * np.sqrt(np.prod(input_shape))
    ...
    for idx in range(len(bins) - 1):
        ...
        indices = torch.cat([indices, index], dim=0)
    ...
    for step in range(1, 1 + particle_gradient_steps):
        ...
        next_beam = torch.stack(next_beam).transpose(0, 1) # shape: (num_evaluation_samples, candidate_directions, input_dim)
        candidate_directions, input_dim = next_beam.shape[1], next_beam.shape[2]
        next_beam = next_beam.reshape(-1, input_dim)

        # next_scores = HartmannOracle(next_beam)
        ...
        next_beam = next_beam.reshape(-1, candidate_directions, input_dim)
        next_scores = next_scores.reshape(-1, candidate_directions)
        index = torch.sort(next_scores, dim=1, descending=True)[1][:,:10]
        current_beam = torch.gather(next_beam, 1, index.unsqueeze(-1).expand(-1, -1, input_dim))
        current_beam = current_beam.transpose(0, 1)
        xt = current_beam[0]
    ...
    with torch.no_grad():
        for (x, _) in loader:
            x = x.to(device).float()
            retrieval_set = trainer.vanilla_search_engine(x, pool_data)
