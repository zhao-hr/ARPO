...
class HartmannTask(object):
    def __init__(self, data_dir, ** task_kwargs) -> None:
        ...
        self.x, self.y = self.clip_outliers(self.x, self.y, top_k=1000, bottom_k=1000)
    ...
    @staticmethod
    def clip_outliers(x, y, top_k=10, bottom_k=10) -> Tuple[np.ndarray, np.ndarray]:
        """
        clip outliers on cpu, delete max k samples & min k samples.
        """
        assert x.shape[0] == y.shape[0]
        _y = torch.tensor(y, dtype=torch.float32)
        indices_min = torch.topk(-_y[:, 0], k=bottom_k)[1]
        indices_max = torch.topk(_y[:, 0], k=top_k)[1]
...
def build_classifier_pipeline(x, y, batch_size=128, train_ratio=0.6):
